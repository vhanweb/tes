            ...
            using (POSContext context = new POSContext())
            {
                ...
                result = (from a in context.TAdjusmentStock
                          ...
                          select new AdjustmentViewModel()
                          {
                              ...
                              SKU = iv.SKU,
                              Adjusment = ii.Adjusment,               //asd.ActualStock +ii.Beginning,
                              OutletID = a.OutletID,
                          }).ToList();
            }
            return result;
        }
        public static List<AdjustmentStockDetailViewModel> GetDataVarName()
        {
            List<AdjustmentStockDetailViewModel> result = new List<AdjustmentStockDetailViewModel>();
            using (POSContext context = new POSContext())
            {
                result = (from asd in context.TAdjusmentStockDetail
                          join iv in context.TItemsVariant on asd.VariantID equals iv.ID into ivTmp
                          from iv in ivTmp.DefaultIfEmpty()
                          select new AdjustmentStockDetailViewModel()
                          {
